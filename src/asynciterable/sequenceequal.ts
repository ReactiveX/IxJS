...
export async function sequenceEqual<T>(
  ...
  const it1 = wrapWithAbort(source, opts.signal)[Symbol.asyncIterator]();
  const it2 = wrapWithAbort(other, opts.signal)[Symbol.asyncIterator]();
  let next1: IteratorResult<T>;
  let next2: IteratorResult<T>;
  while (!(next1 = await it1.next()).done) {
    if (!(!(next2 = await it2.next()).done && (await opts.comparer(next1.value, next2.value)))) {
      return false;
    }
  }
