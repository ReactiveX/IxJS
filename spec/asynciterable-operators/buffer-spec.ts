...
test('AsyncIterable#bufferPipe no skip non-full buffer', async t => {
  const rng = range(0, 10);

  const res = await toArray(rng.pipe(bufferPipe(3)));
  t.equal(4, res.length);
...
test('AsyncIterable#bufferPipe no skip all full', async t => {
  const rng = range(0, 10);

  const res = await toArray(rng.pipe(bufferPipe(5)));
  t.equal(2, res.length);
...
test('AsyncIterable#bufferPipe no skip empty buffer', async t => {
  const rng = empty<number>();
...
test('AsyncIterable#bufferPipe skip non-full buffer', async t => {
  const rng = range(0, 10);

  const res = await toArray(rng.pipe(bufferPipe(3, 2)));
  t.equal(5, res.length);
...
test('AsyncIterable#bufferPipe skip full buffer', async t => {
  const rng = range(0, 10);

  const res = await toArray(rng.pipe(bufferPipe(3, 4)));
  t.equal(3, res.length);
