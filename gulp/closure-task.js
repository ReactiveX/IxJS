...
export const closureTask = ((cache) =>
  memoizeTask(cache, async function closure(target, format) {
    ...
    async function closureCompile(entry) {
      ...
      return await observableFromStreams(
        ...
        sourcemaps.init(),
        closureCompiler(
          createClosureArgs(target, entry_point, entry, externsPath, getUMDExportName(entry)),
          {
...
function generateExternsFile(entryModulePath) {
  ...
  return [
    externsHeader(),
    ...getPublicExportedNames(entryModule).map(externBody).filter(Boolean),
  ].join('\n');
}
...
function getPublicExportedNames(entryModule) {
  ...
  const isStaticOrProtoName = (x) =>
    !(x in fn) && x !== `default` && x !== `undefined` && x !== `__esModule` && x !== `constructor`;
  return Object.getOwnPropertyNames(entryModule)
    .filter((name) => name !== 'default')
    .filter(
      (name) => typeof entryModule[name] === `object` || typeof entryModule[name] === `function`
    )
    .map((name) => [name, entryModule[name]])
    .reduce((reserved, [name, value]) => {
      const staticNames =
        value && typeof value === 'object'
          ? Object.getOwnPropertyNames(value).filter(isStaticOrProtoName)
          : typeof value === 'function'
          ? Object.getOwnPropertyNames(value).filter(isStaticOrProtoName)
          : [];

      const instanceNames = (
        (typeof value === `function` && Object.getOwnPropertyNames(value.prototype || {})) ||
        []
      ).filter(isStaticOrProtoName);
